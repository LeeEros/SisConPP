// prisma/seeds/peaoAdulto.seed.ts
import { PrismaClient, VivenciaSubGrupo } from "@prisma/client";

export async function seedPeoes(prisma: PrismaClient) {
  ...
  const vivencia = await prisma.quesitos.create({
    data: {
      ...
      opcional: false,
      blocoProvaIdBloco: provaOral.idBloco,
    },
  });
  ...
  await prisma.subQuesitos.createMany({
    data: [
      {
        nomeSubquesito: "Conteúdo da pesquisa escrita",
        notaSubequesito: 2.0,
        quesitoId: pesquisa.idQuesito,
      },
      {
      ...
      {
        nomeSubquesito: "Tema compatível com a categoria",
        notaSubequesito: 0.5,
        quesitoId: pesquisa.idQuesito,
      },
      {
        nomeSubquesito: "Estrutura da pesquisa escrita (formatação e referências)",
        notaSubequesito: 2.0,
        quesitoId: pesquisa.idQuesito,
      },
      {
        nomeSubquesito: "Desenvoltura e Expressão durante a explicação da pesquisa",
        notaSubequesito: 2.0,
        quesitoId: pesquisa.idQuesito,
      },
      {
        nomeSubquesito: "Criatividade / recurso utilizado na apresentação",
        notaSubequesito: 2.0,
        quesitoId: pesquisa.idQuesito,
      },
    ],
  ...
  const dancadesalao = await prisma.quesitos.create({
    data: {
        nomeQuesito: "Dança de Salão",
        notaMaximaQuesito: 19,
        opcional: false,
        blocoProvaIdBloco: provaArtistica.idBloco,
    },
  });
        ...
        data: [
         {
            ...
            notaSubequesito: 0.5,
            quesitoId: dancadesalao.idQuesito,
        },
        {
    ...
    await prisma.subQuesitos.createMany({
     ...
     const provaEsportiva= await prisma.quesitos.create({
        data: {
            nomeQuesito: "Prova Esportiva Regulamentada",
            notaMaximaQuesito: 6.0,
            opcional: true,
            blocoProvaIdBloco: provaArtistica.idBloco,
        },
    ...
    await prisma.subQuesitos.createMany({
        data: [
            {
                nomeSubquesito: "Conhecimento sobre a modalidade escolhida",
                notaSubequesito: 2.0,
                quesitoId: provaEsportiva.idQuesito,
            },
            {
                nomeSubquesito: "Explicação técnica da modalidade",
                notaSubequesito: 2.5,
                quesitoId: provaEsportiva.idQuesito,
            },
            {
                nomeSubquesito: "Demostração prática",
                notaSubequesito: 1.5,
                quesitoId: provaEsportiva.idQuesito,
            }
        ] 
    ...
    const encilhar = await prisma.quesitos.create({
        data: {
            ...
            notaMaximaQuesito: 6.0,
            opcional: false,
            blocoProvaIdBloco: provaCampeira.idBloco,
        },
    });
    ...
    await prisma.subQuesitos.createMany({
        data: [
            ...
            {
                nomeSubquesito: "Domínio teórico da origem e função das peças de encilha", 
                notaSubequesito: 2.0,
                quesitoId: encilhar.idQuesito,
            },
            {
                nomeSubquesito: "Domínio da prática (ordem das peças e montagem da encilha)",
                notaSubequesito: 2.0,
                quesitoId: encilhar.idQuesito,
            }
        ]
        ...
        data: [
            {
                ...
                notaSubequesito: 1.0,
                quesitoId: lacarCavalo.idQuesito,
            },
            {
        ...
        data: [
            {
                nomeSubquesito: "Conhecimento sobre a prova escolhida",
                notaSubequesito: 1.0,
                quesitoId: provasderedeas.idQuesito,
            },
            {
                nomeSubquesito: "Explicação da técnica a ser apresentada",
                notaSubequesito: 1.0,
    ...
    const culinariaCampeira = await prisma.quesitos.create({
        data: {
            nomeQuesito: "Culinária Campeira",
            notaMaximaQuesito: 4.0,
            opcional: true,
            blocoProvaIdBloco: provaCampeira.idBloco,
        },
        ...
        data: [
            {
                nomeSubquesito: "Conhecimento sobre a prova escolhida",
                notaSubequesito: 1.0,
                quesitoId: culinariaCampeira.idQuesito,
            },
            {
                nomeSubquesito: "Explicação da técnica a ser apresentada",
                notaSubequesito: 1.0,
